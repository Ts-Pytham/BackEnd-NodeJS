...
export const registrarMaquina = async (req, res) => {
    try {
        ...
        const { nombre, marca, placa, modelo, cantidad, serial, descripcion, estado, id_usuario, id_area, id_ambiente,estado_maquina } = req.body;
        let imagen = "";
        let manual = "";
        ...
        const sql = `INSERT INTO maquina(nombre, marca, placa, modelo, cantidad, manual, serial, imagen, descripcion, estado, id_usuario, id_area, id_ambiente, estado_maquina)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`;

        const values = [nombre, marca, placa, modelo, cantidad, manual, serial, imagen, descripcion, estado, id_usuario, id_area, id_ambiente,estado_maquina];
...
export const listarMaquinas = async (req, res) => {
    try {
        const [result] = await pool.query('SELECT usuario.nombres AS nombre_usuario , area.nombre AS nombre_area, ambiente.nombre AS nombre_ambiente, maquina.nombre AS nombre_maquina , maquina.estado AS estado_maquinaa, maquina.estado_maquina,id_maquina,  marca, placa, modelo, cantidad, manual, serial, imagen, descripcion FROM usuario INNER JOIN  maquina ON usuario.id_usuario = maquina.id_usuario INNER JOIN  area ON area.id_area = maquina.id_area INNER JOIN ambiente ON ambiente.id_ambiente = maquina.id_ambiente;');
...
export const actualizarMaquina = async (req, res) => {
    try {
        ...
        const imagen = req.files.img[0].originalname;
        const manual = req.files.manual[0].originalname;

        const sql = `
            UPDATE maquina
            SET nombre=?, marca=?, placa=?, modelo=?, cantidad=?, manual=?, serial=?, imagen=?, descripcion=?, estado=?, id_usuario=?, id_area=?, id_ambiente=?,estado_maquina=?
            WHERE id_maquina = ?
        `;

        const values = [nombre, marca, placa, modelo, cantidad, manual, serial, imagen, descripcion, estado, id_usuario, id_area, id_ambiente,estado_maquina, id];
